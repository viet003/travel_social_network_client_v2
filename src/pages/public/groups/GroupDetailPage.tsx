...
import { apiGetGroupById, apiJoinGroup, apiLeaveGroup, apiUpdateGroup, type GroupResponse } from '../../../services/groupService';
...
const GroupDetailPage: React.FC = () => {
  ...
  // Redirect to about tab if user is not a member
  useEffect(() => {
    if (groupData && !groupData.isMember && activeTab !== 'about') {
      navigate(`/home/groups/${groupId}/${path.GROUP_ABOUT}`, { replace: true });
    }
  ...
  const handleJoinLeave = async () => {
    ...
    try {
      ...
      toast.error(groupData.isMember ? 'Không thể rời khỏi nhóm' : 'Không thể tham gia nhóm');
    } finally {
      setIsJoining(false);
    ...
    <div className="min-h-screen bg-white">
      <div>
        <div className="relative h-[300px] sm:h-[400px] lg:h-[500px] rounded-b-xl bg-gray-200 max-w-[1200px] mx-auto overflow-hidden">
          <img 
            src={groupData.coverImageUrl || 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=1200&h=400&fit=crop'} 
          ...
          {(groupData.currentUserRole === 'OWNER') && (
            <div className="absolute z-30 flex items-center gap-2 top-3 left-3 sm:top-6 sm:left-6">
              <Tooltip title="Chỉnh sửa nhóm" placement="bottom">
                ...
                >
                  <Icon icon="lucide:edit-3" width={14} className="sm:w-4 sm:h-4" />
          ...
          <div className="max-w-[1200px] mx-auto px-3 sm:px-6 lg:px-[50px]">
            <div className="flex items-end justify-between py-4">
              <div className="flex items-end space-x-4">
                <div className="pb-2">
                  <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">{groupData.groupName}</h1>
                  <div className="flex items-center space-x-2 text-xs sm:text-sm text-gray-600">
                    <Icon icon={!groupData.privacy ? "fluent:globe-24-filled" : "fluent:lock-closed-24-filled"} className="h-4 w-4" />
                    <span>{!groupData.privacy ? 'Nhóm công khai' : 'Nhóm riêng tư'}</span>
                    <span>•</span>
              ...
              <div className="flex items-center space-x-2 sm:space-x-3 pb-2 flex-wrap sm:flex-nowrap">
                {groupData.friendMembers && groupData.friendMembers.length > 0 && (
                  <div className="flex -space-x-2 mr-2">
                    {groupData.friendMembers.slice(0, 5).map((member, index) => (
                      <img 
                        key={member.userId} 
                        ...
                        onClick={() => navigate(`/home/user/${member.userId}`)}
                        title={member.name}
                      />
                    ))}
                  </div>
                  ...
                  <>
                    <div className="relative member-dropdown-container">
                      ...
                      >
                        ...
                        <Icon icon="fluent:chevron-down-24-filled" className="h-3 w-3 sm:h-4 sm:w-4" />
                      </button>
                            ...
                            {groupData.currentUserRole === 'OWNER' ? (
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <div className="text-xs text-gray-500">Xóa vĩnh viễn nhóm này</div>
                                </div>
                              </button>
                            ) : (
                              <button
                              ...
                              >
                                <Icon icon="fluent:door-arrow-left-24-regular" className="h-5 w-5" />
                                <div>
                                  <div className="font-medium">Rời nhóm</div>
                                  <div className="text-xs text-gray-500">Bạn sẽ không còn là thành viên</div>
                                </div>
                              </button>
                            )}
                          </div>
      ...
      <div className="px-3 sm:px-6 lg:px-[50px] py-4 sm:py-6 lg:py-8 mx-auto max-w-[1200px]">
        <Outlet context={{ groupData, isMember: groupData?.isMember || false }} />
      </div>
      ...
      <ConfirmDeleteModal
        ...
        showStats={true}
        stats={[
          { icon: 'fluent:people-24-regular', label: 'thành viên', value: groupData?.memberCount || 0 },
          { icon: 'fluent:document-24-regular', label: 'bài viết', value: groupData?.postsLastMonth || 0 },
        ]}
      />
