...
const PostCreateModal: React.FC<PostCreateModalProps> = ({
  ...
  const { userId, avatar, firstName, lastName } = useSelector(
    ...
    ? [
        {
          ...
          label: "Công khai",
          icon: () => (
            <Icon icon="fluent:globe-24-filled" className="w-4 h-4" />
          ),
  ...
  const handleMediaSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      if (!allowedTypes.includes(file.type)) {
        toast.error(
          "Chỉ hỗ trợ tệp hình ảnh (JPEG, PNG, GIF) và tệp video (MP4, MOV, AVI)!"
        );
        return;
      }
      ...
      if (
        ...
        toast.error(
          "Bạn không thể trộn video với hình ảnh. Vui lòng xóa tất cả hình ảnh trước hoặc chỉ chọn hình ảnh."
        );
        return;
      }
      ...
      if (
        !isVideo &&
        selectedMedia.some((media) => media.type.startsWith("video/"))
      ) {
        toast.error(
          "Bạn không thể trộn hình ảnh với video. Vui lòng xóa video trước hoặc chỉ chọn video."
        );
        return;
      }
  ...
  return (
    <>
      ...
      >
        ...
        >
          <TravelButton type="primary" onClick={handleOpen} className="!p-2">
            ...
            >
              ...
              <div className="flex items-center gap-2 p-2 rounded-lg bg-blue-50">
                ...
                <span className="text-xs text-blue-700">
                  <strong>{currentPrivacy?.label}:</strong>{" "}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-gray-600">
                        {selectedMedia.length} {hasVideo ? "video" : "hình ảnh"}{" "}
                        đã chọn
                      </span>
                      <button
                        type="button"
                          ...
                          <div
                            ...
                            className={`relative border border-gray-200 rounded-lg overflow-hidden ${
                              selectedMedia.length === 1
                                ...
                                ? "h-[200px]"
                                : selectedMedia.length === 3 && index === 0
                                ? "col-span-2 h-[200px]"
                                : "h-[150px]"
                            }`}
                          >
