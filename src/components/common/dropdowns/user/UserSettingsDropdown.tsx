      ...
      <div className="absolute right-0 mt-2 w-72 bg-white border border-gray-200 rounded-2xl shadow-xl z-20 overflow-hidden">
        <div className="py-2">
          ...
          {isBlocked ? (
            <button
              onClick={onUnblock}
              className="w-full flex items-center space-x-3 py-2 px-3 rounded-xl hover:bg-blue-50 transition-colors cursor-pointer group"
            >
              <div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center">
                <Icon icon="lucide:user-check" className="w-5 h-5 text-blue-600" />
              </div>
              <div className="flex-1 text-left">
                <p className="text-sm font-medium text-blue-600">Bỏ chặn người dùng</p>
                <p className="text-xs text-blue-500">Cho phép họ xem nội dung của bạn</p>
              </div>
            </button>
          ) : (
            <button
